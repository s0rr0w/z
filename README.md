...
Данная директива запускает диспатчеры, которые содержит данный тег.
...
* `templatingMode` -- режим темплейтирования `add`, `replace` или `once`
...
В режиме `add` контент будет добавлен к уже существующему контенту в контейнере, который содердит данный обработчик. В режиме `replace` контент контейнера будет заменен на новый. Режим `once` схож с `replace`, только темплейтирование будет произведено всего один раз.
...
<div>
 <div>
  ...
  <!-- Этому темплейту будут переданы данные { "one": { "x": 0, "y": 50 } } -->
 </div>
 <div>
...
Шаблонизатор использует следующие конструкции
...
* [include](#Вставка-темплейтов-include)
...
Если выполняется условие `expression`, темплейтируется код в `<then>`, иначе `<else>`. В случаях, когда `<else>` отсутствует, `<then>` можно не указывать, тогда темплейтироваться будет код в `<if>`.
...
При темплейтировании следующих данных
```json
{
 "user": {}
}
